...
private:
    ...
    void drawFrame() {
        ...
        // frame 2: image 2 + wait(fence 0) + vkQueueSubmit(cmd, fence 0) // cpu等frame 0的fence, gpu做完了再画.
        // frame 3: image 0 + wait(fence 1) + vkQueueSubmit(cmd, fence 1) // cpu等frame 1的fence, gpu做完了再画.
        // frame 4: image 1 + wait(fence 0) + vkQueueSubmit(cmd, fence 0) // cpu等frame 2的fence, gpu做完了再画.
        // frame 5: image 2 + wait(fence 1) + vkQueueSubmit(cmd, fence 1) // cpu等frame 3的fence, gpu做完了再画.
        //
        // 如果是3个images和3个fence:
        // frame 0: image 0 + wait(fence 0) + vkQueueSubmit(cmd, fence 0) // fence没用过不用等
        // frame 1: image 1 + wait(fence 1) + vkQueueSubmit(cmd, fence 1) // fence没用过不用等
        ...
        //
        // 总结: swapchain iamge = minCount + 1 = 3, 是推荐的做法.
        ...
        std::cout << "drawFrame(), imageIndex: " << imageIndex << ", currentFrame: " << currentFrame << std::endl;
    ...
    QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
        ...
        for (size_t i = 0; i < queueFamilyCount && printOnce; i++) {
            ...
            std::cout << "queueFamily[" << i << "]:\n"
                      << "    queueFlags: 0x" << std::hex << queueFamilies[i].queueFlags << std::endl
                      << "    queueCount=" << queueFamilies[i].queueCount << std::endl
                      << "    timestampValidBits=" << queueFamilies[i].timestampValidBits << std::endl
                      << "    minImageTransferGranularity[w,h,d]="
    ...
    bool checkValidationLayerSupport() {
        ...
        for (const auto& layerProperties : availableLayers) {
            std::cout << "  layerName: " << layerProperties.layerName << std::endl
                      << "  version: " << layerProperties.specVersion << std::endl
                      << "  implementationVersion: " << layerProperties.implementationVersion << std::endl
                      << "  description: " << layerProperties.description << std::endl
                      << std::endl;
        }
