...
private:
    ...
    void initWindow() {
        ...
        glfwSetCharCallback(window, characterCallback);  // for future use
    ...
    static void characterCallback(GLFWwindow* window, unsigned int codepoint) {
        ...
        if ('b' == codepoint) {
            app->useBVH = 1.0 - app->useBVH;
            std::cout << "useBVH: " << app->useBVH << std::endl;
        }
    }
    ...
    void createShaderStorageBuffers() {
        // spheres buffer
        {
            VkDeviceSize bufferSize = sizeof(Sphere) * bvh.getSpheres().size();
            ...
            memcpy(data, bvh.getSpheres().data(), (size_t)bufferSize);
    ...
    void recordComputeCommandBuffer(VkCommandBuffer commandBuffer) {
        ...
        vkCmdDispatch(commandBuffer, (WIDTH + 31) / 32, (HEIGHT + 31) / 32, 1);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.camFront = camera.Front;
        ubo.sphereCount = static_cast<float>(bvh.getSpheres().size());
        ubo.camUp = camera.Up;
        ubo.useBVH = this->useBVH;
        ubo.camRight = camera.Right;
