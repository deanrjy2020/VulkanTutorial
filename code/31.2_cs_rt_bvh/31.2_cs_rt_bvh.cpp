...
private:
    ...
    void createLogicalDevice() {
        ...
        for (uint32_t queueFamily : uniqueQueueFamilies) {
            ...
            // TODO(review): 固定申请 3 个 queue 可能超过该 family 的 queueCount；本机仅支持 1 个，已触发 validation error。
            // 后续按实际数量分配，不足时复用 queue；同时确认 present 使用的 family 确实支持 presentation。
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        if (currentStorageImageLayout[currentFrame] == VK_IMAGE_LAYOUT_GENERAL) {
            ...
            vkCmdPipelineBarrier(
                commandBuffer,
                // TODO(review): graphics 在 FRAGMENT_SHADER 等 computeFinished，TOP_OF_PIPE 未衔接该等待。
                // 布局转换可能与 compute 写入重叠；后续让 barrier 源阶段衔接等待阶段，或将 semaphore 等待提前。
    ...
    void drawFrame() {
        ...
        // TODO(review): 后续 acquire 若返回 OUT_OF_DATE，会直接返回，导致此 fence 无人 signal，下一帧永久等待。
        // 此时已提交的 compute 还会留下未消费的 binary semaphore；后续应先成功 acquire，再 reset/提交。
        ...
        submitInfo.signalSemaphoreCount = 1;
        // TODO(review): graphics fence 不保证 present 已用完 semaphore，按 frame slot 复用可能再次 signal 仍在使用的对象。
        // 后续参考 27.1：按 swapchain image 分配并以 imageIndex 使用；31.1 和 40 示例也有同类问题。
